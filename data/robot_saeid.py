...
import os
import numpy as np
#EDIT I changed utils.py to utils2.py to avoid conflicts with the yolo folder utils
import utils2
from simulation import vrep


# EVERYTHING THAT IS ADDED/CHANGED FROM THE ORIGINAL ROBOT_SAEID FILE
# IS MARKED WITH 'EDIT'

#EDIT adding torch, pillow, and json for object classification
...
class Robot(object):
    def __init__(self, is_sim, workspace_limits):
    ...
    #EDIT added from original pretrained_torch file and editted to work here
    def pretrained_torch(self, rgb_data):
        ...
        #EDIT making an image from the rgb data
        ...
        # PUT THE FILES WHERE THE COMMAND LINE IS EXECUTING FROM!!!!!!!!
        # Otherwise it will throw a file not found error
        # You will not always be running the files from the same folder they are present in
    ...
    def setup_sim_camera(self):
        ...
        cam_rotm = np.eye(4,4)
        cam_rotm[0:3,0:3] = np.linalg.inv(utils2.euler2rotm(cam_orientation))
    ...
    def restart_sim(self):
        ...
        #sim_ret, self.UR5_target_handle = vrep.simxGetObjectHandle(self.sim_client,'UR5',vrep.simx_opmode_blocking)
...
#             # Now close the connection to V-REP:
 #           vrep.simxFinish(self.sim_client)
    ...
    #added destination so the object can be placed in a new location
    def grasp(self, position, heightmap_rotation_angle, workspace_limits, destination):
        ...
        if self.is_sim:
            ...
            if grasp_success:
                #EDIT, commentted out original code and added my own
                #taken from a place function elsewhere in this file
            ...
            tool_orientation_rotm = utils2.angle2rotm(tool_orientation_angle, tool_orientation_axis, point=None)[:3,:3]

            # Compute tilted tool orientation during dropping into bin
            tilt_rotm = utils2.euler2rotm(np.asarray([-np.pi/4,0,0]))
            tilted_tool_orientation_rotm = np.dot(tilt_rotm, tool_orientation_rotm)
            tilted_tool_orientation_axis_angle = utils2.rotm2angle(tilted_tool_orientation_rotm)
    ...
    def push(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if self.is_sim:
            ...
            tilt_axis = np.dot(utils2.euler2rotm(np.asarray([0,0,np.pi/2]))[:3,:3], push_direction)
            tilt_rotm = utils2.angle2rotm(-np.pi/8, tilt_axis, point=None)[:3,:3]
            tilted_tool_orientation_rotm = np.dot(tilt_rotm, tool_orientation_rotm)
            tilted_tool_orientation_axis_angle = utils2.rotm2angle(tilted_tool_orientation_rotm)
            tilted_tool_orientation = tilted_tool_orientation_axis_angle[0]*np.asarray(tilted_tool_orientation_axis_angle[1:4])
